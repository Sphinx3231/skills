// Single source of truth for CLIP zero-shot's candidate label set.
//
...
// or the confirm screen's `foodName` field (see local-food-analysis.js,
// which matches a result back to its `key` by `prompt`, once, in one place).
//
// This exact 36-food + 3-anchor set is carried over from the CLIP spike
// (docs/outcomes/clip-zero-shot-spike-findings.md) that this ticket's plan
// is built on — same labels, same "a photo of X" template for foods, same
// full-sentence anchors. Reusing the spike's exact set (rather than a fresh
// list) means the plan's cited accuracy numbers (91.7% in-vocab, 2/4
// non-food correctly rejected) describe the actual candidate set shipping
// here, not a superficially similar one.
